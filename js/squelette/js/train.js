...
function wagonPb(plateau, n, x, y) {
  let a = 0;
  for (let i = 0; i < n; ++i) {
    if (plateau.cases[x - i][y].nom.includes("rail")) {
      ++a;
    }
  }
  console.log(a);
...
function avancer(contexte, plateau) {
  ...
  sncf.forEach((train) => {
    ...
    if (
      ...
      //console.log(plateau.cases[train.x][train.y].nom);
      if (plateau.cases[train.x][train.y].nom.includes("vers bas")) {
        if (train.direction === H) {
          train.direction = G;
        } else if (train.direction.includes(D)) {
          train.direction = B;
        } else {
          dehors.push(train);
        }
      }
      if (plateau.cases[train.x][train.y].nom.includes("vers haut")) {
        if (train.direction === D) {
          train.direction = H;
        } else if (train.direction.includes(B)) {
          train.direction = G;
        } else {
          dehors.push(train);
        }
      ...
      if (
        ...
        if (train.direction === H) {
          train.direction = D;
        } else if (train.direction.includes(G)) {
          train.direction = B;
        } else {
          dehors.push(train);
        }
    ...
    i.addEventListener("click", () => {
  	...
  	bp.addEventListener("click",(e)=>{
  		if(bp.innerText=="Pause"){
  			...
  			bp.innerText="Pause";
  			pause=false;
  			setTimeout(avancer, 500, contexte, plateau);
  		}
  	});
